...
int main(int argc, char **argv) {
  if (argc - 1 < NUM_ARGS) {
    cout << "Usage: judge cachePath binaryName binaryPath lang input timeLimit "
            "directoryPathSeparator runScriptBinaryPath checker epsilon "
            "customCheckerPath\n";
    return 1;
  }
  ...
  string inputPath =
      cachePath + directoryPathSeparator + binaryName + "-customInvocation.in";
  ofstream out(inputPath);
  out << input << "\n";
  out.close();
  string userOutputPath = cachePath + directoryPathSeparator + binaryName +
                          "-customInvocation.user.out";
  ...
  string run = runScriptBinaryPath + " " + cachePath + " " + binaryPath + " " +
               binaryName + " " + lang + " " + inputPath + " " +
               userOutputPath + " " + timeLimit + " 2>&1 >/dev/null";
  ...
  if (fgets(buf, BUF_SIZE, runOutput) != NULL) {
    ...
    if (stdErrSanitized.find(CPU_SOFT) != string::npos ||
        stdErrSanitized.find(CPU_HARD) != string::npos)
      runStatus = TLE;
    else
      runStatus = RTE;
  }
  ...
  // Only check if a checker is provided
    ...
    string check = apolloLaunch + " " + inputPath + " " + userOutputPath + " " +
                   userOutputPath; // Custom invocation should not need a
                                   // defined output... its custom
